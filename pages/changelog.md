...
2016-11-07 // Version 2.0
**ToDo**
:   *Feeling Responsive* is now based on Foundation 6
:   Typography selection in `config.yml`
:   Color Scheme selection in `config.yml`
:   Color Scheme now more minimalistic
:   Kicked Backstretch.js and solve it with pure CSS
...
:   Instead of Entypo *Feeling Responsive* now uses SVG-icons
:   Now search is powered by lunr.js (optionally use Google Searc)
:   Kicked own variable {% raw %}`{{ site.urlimg }}`{% endraw %} to make it easier
:   Instead of {% raw %}`{{ site.url }}{{ site.baseurl }}`{% endraw %} we use the new `absolure_url`-filter like this {{ page.url | absolute_url}}
